...
const ServiceItem = ({ title, count, icon: Icon, onClick, color = 'brand-primary' }: any) => (
    <div onClick={onClick} className="glass-card rounded-[2.5rem] p-6 border-white/5 active:scale-[0.97] transition-all flex items-center gap-6 group">
        <div className={`w-16 h-16 bg-${color}/10 rounded-3xl flex items-center justify-center text-${color} border border-${color}/20 group-hover:scale-110 transition-transform`}>
            <Icon className="w-8 h-8" />
        </div>
        <div className="flex-1">
            <h3 className="text-xl font-black text-white tracking-tight">{title}</h3>
            <p className={`text-[10px] font-black uppercase tracking-[0.2em] text-${color}`}>{count} Services Available</p>
        </div>
